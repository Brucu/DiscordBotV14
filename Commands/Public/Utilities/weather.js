...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(options => options
            .setName("location")
            .setDescription("The location to check the weather are of")
            .setRequired(true)
        )
        .addStringOption(options => options
            .setName("degree-type")
    ...
    async execute(interaction) {
        ...
        await interaction.reply({ content: `<a:check_mark:1102229851588538368> Gathering your weather data...` });

        await weather.find({ search: `${location}`, degreeType: `${degree}` }, async function (err, result) {
            setTimeout(() => {
                if (err) {
                    console.log(err);
                    interaction.editReply({ content: `${err} | Beacuse we are pulling data, sometimes timeouts happen! Try this command again` });
                } else {
                    if (result.length == 0) {
                        ...
                        const embed = new EmbedBuilder()
                            ...
                            .addFields({ name: "🌡️ Temperature", value: `${temp}` })
                            .addFields({ name: "🙂 Feels Like", value: `${feel}` })
                            .addFields({ name: "☁️ Weather", value: `${type}` })
